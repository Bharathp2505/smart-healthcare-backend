...
@Service
public class UseroldService implements UserDetailsService {
    ...
    public void registerUser(final UserRequest userRequest) {
        if (useroldRepository.existsByEmailId(userRequest.getEmailId())) {
        ...
        if (useroldRepository.existsByPhoneNumber(userRequest.getPhoneNumber())) {
    ...
    public UserDetails loadUserByUsername(String emailId) throws UsernameNotFoundException {
        final Userold user = Optional.ofNullable(useroldRepository.findByEmailId(emailId)).orElseThrow(() -> new UsernameNotFoundException("User not found"));
